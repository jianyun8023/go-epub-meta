...
var (
	inputFile = flag.String("i", "full_comparison_results.csv", "Input CSV file")
	samples   = flag.Int("samples", 20, "Number of sample mismatches to show")
)
	...
	for _, r := range records {
		if r.GolibriAuthor != "" && r.EbookAuthor != "" {
			if normalize(r.GolibriAuthor) != normalize(r.EbookAuthor) {
				authorMismatches++
				if len(sampleMismatches) < *samples {
